...
import { User, Palette, Settings, Layout, Check, ChevronDown, Code2, BarChart3, Tags, Zap, Trophy, PieChart, GripVertical, Eye, EyeOff, Boxes, Layers, Sparkles, Shield, Diamond, Brush, Search, Trash2, Copy, Share2, Disc, MessageSquare, Plus, Camera, Send, Play, Briefcase, Code } from "lucide-react";
                                          ...
                                          <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                              ...
                                              {skill.isAuto ? (
                                                <button 
                                                  onClick={() => {
                                                    // Handle both languages and skills (topics)
                                                    if (store.autoLanguages.some(l => l.name === skill.name)) {
                                                      store.toggleLanguageVisibility(skill.name);
                                                    } else {
                                                      store.toggleSkillVisibility(skill.name);
                                                    }
                                                  }}
                                                  className={cn(
                                                    "p-1.5 rounded-lg transition-all",
                                                    !(store.hiddenLanguages.includes(skill.name) || store.hiddenSkills.includes(skill.name)) 
                                                      ? "bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500 hover:text-white"
                                                      : "bg-slate-500/10 text-slate-500 hover:bg-slate-500 hover:text-white"
                                                  )}
                                                >
                                                  {!(store.hiddenLanguages.includes(skill.name) || store.hiddenSkills.includes(skill.name)) ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}
                                                </button>
                                              ) : (
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                   {store.socialProfiles.map((profile) => {
                     const Icon = profile.platform === 'youtube' ? Play :
                                profile.platform === 'discord' ? Disc :
                                profile.platform === 'twitter' ? Send :
                                profile.platform === 'instagram' ? Camera :
                                profile.platform === 'linkedin' ? Briefcase :
